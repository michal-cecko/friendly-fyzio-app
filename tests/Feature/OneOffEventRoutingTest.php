...
class OneOffEventRoutingTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_wrong_category_url_redirects_to_the_canonical_permalink(): void
    {
        ...
        $event = OneOffEvent::factory()->forCategory($workshopy)->published()->create([
            'event_date' => today()->addWeeks(2)->toDateString(),
        ]);
    ...
    public function test_old_lesson_url_redirects_to_the_event_permalink_by_preserved_id(): void
    {
        ...
        $event = OneOffEvent::factory()->forCategory($category)->withCourse($course)->published()->create([
            'event_date' => today()->addWeeks(2)->toDateString(),
        ]);
