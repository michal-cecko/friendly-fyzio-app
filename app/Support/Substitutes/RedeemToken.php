...
class RedeemToken
{
    ...
    public function __invoke(SubstituteToken $token, CourseLesson $target): LessonAttendance
    {
    ...
    protected function persist(SubstituteToken $token, CourseLesson $target): LessonAttendance
    {
        ...
        if (! $this->options->forToken($token)->contains(fn (CourseLesson $lesson): bool => $lesson->is($target))) {
