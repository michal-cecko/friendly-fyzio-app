...
<head>
    ...
    @php($seoTitle = ($page->meta_title ?? null) ?: ($page->title ?? config('app.name')))
    @php($ogImage = isset($page) ? \App\Support\Media::url($page->featured_image, '800') : null)
...
<body class="flex min-h-screen flex-col bg-white font-sans text-neutral-900 antialiased">
    <x-site.banners :page-id="$page->id ?? null" />
