...
<div class="rounded-xl border-2 border-amber-300 bg-amber-50 p-5">
    ...
    <p class="mt-1 text-sm text-[#666666]">{{ $gate === 'email_exists' ? 'K zadanému e-mailu už existuje účet. Přihlaste se prosím, ať nevznikne duplicitní účet.' : 'Přihlaste se prosím, abychom ověřili, že jste naším pacientem.' }}</p>
    <div class="mt-4 grid gap-3">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if ($gate === 'email_exists')
                <button type="button" wire:click="useDifferentEmail" class="rounded-full border border-amber-300 bg-white px-6 py-3 font-heading text-[15px] font-semibold text-amber-900 transition hover:brightness-95">Použít jiný e-mail</button>
            @endif
